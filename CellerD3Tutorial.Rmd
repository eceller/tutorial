...
Basically, D3.js is simply one of the many ways we are able to produce a visualization; it is essentially like drawing.  D3 is a javascript library that is predominately used to create interactive data visualizations in web browsers.  Unlike R, learning D3.js requires learning Javascript.  With that in mind, it is still possible to produce the same results with both programs (for the most part), one simply requires knowledge of coding with HTML, CSS, etc, while the other requires knowledge of coding with R.  D3 is used by a variety of organizations for data visualization, ranging from The New York Times to MTV to academia and policy.

As mentioned above, D3 is one of the many ways of producing data visualizations.  The predominate use of D3 is the production of visulizations.  However, as mentioned above, the visulizations produced with D3 are typically done with HTML, and is not typically something that is executed in R.  
...
The above examples are just a few basic ideas of what D3 can do.  And, the R examples that followed serve to show where, even though D3 might appear to have a more robust ability to produce visualizations, there are several occasions on which we're able to reproduce the results of D3 in R.

There are, however, several more interesting and meaningfully more complex examples of what D3 can do, such as . . . 
